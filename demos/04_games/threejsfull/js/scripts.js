...
// general
let camera1, scene, renderer;

// lights
let ambientlight, directlight1, spotlight1;

// objects
let ground, shape;

// materials
let shapeMaterialPhong, shapeMaterialPhongSmooth, shapeMaterialPhongTexture, shapeMaterialPhongTextureSmooth;
let shapeMaterialLambert, shapeMaterialBasic, shapeMaterialWire;
let shapeGeometry;
...
const init = function() {
	...
	// add ground
	const groundGeometry = new THREE.CubeGeometry(300, 6, 200);
	const groundMaterialPhong = new THREE.MeshPhongMaterial({
		color: 0x666666,
		shininess: 15,
		specular: 0x888888,
	...
	// camera events
	const applyCameraValues = function() {
	...
	const applyShapeMaterial = function() {
		...
		if (rbnMaterialPhong.checked) {
			if (selTexture.value == -1) {
				...
				const newTexture = (new THREE.TextureLoader()).load('img/' + selTexture.value);
